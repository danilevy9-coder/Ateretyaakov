...
export default function DonorsClient({ initialSegment, initialStatus, initialIssueType, onlyIssues }: Props) {
  ...
  const [rows, setRows] = useState<(Donor & { donor_issues?: { type: string; status: string }[] })[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const select = useInner
        ? '*, donor_issues!inner(type,status)'
        : '*, donor_issues(type,status)';
  ...
  }, [supabase, q, segment, status, issueType, issuesOnly, sort, asc, page]);
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        ...
        <a href="/crm/import" className="px-4 py-2 rounded-lg bg-amber-500 hover:bg-amber-400 text-black font-semibold text-sm">
          📥 Import
        </a>
      </div>
        ...
        <table className="w-full text-sm min-w-[820px]">
          ...
          <tbody>
            ...
            {!loading && rows.length === 0 && (
              <tr><td colSpan={8} className="px-4 py-10 text-center text-slate-500">No donors found. Try importing a file.</td></tr>
            )}
            {!loading && rows.map((d) => {
              ...
              return (
                <tr key={d.id} className="border-t border-white/5 hover:bg-white/[0.03]">
                  <td className="px-4 py-2.5">
                    <button onClick={() => setSelected(d)} className="text-white hover:text-amber-300 font-medium text-left">
                      {name}
                    </button>
                  </td>
      ...
      <div className="flex items-center justify-between mt-4 text-sm">
        ...
        <div className="flex gap-2">
          <button disabled={page === 0} onClick={() => setPage((p) => Math.max(0, p - 1))}
            className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 disabled:opacity-40">← Prev</button>
          <button disabled={page + 1 >= totalPages} onClick={() => setPage((p) => p + 1)}
            className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 disabled:opacity-40">Next →</button>
        </div>
      </div>
